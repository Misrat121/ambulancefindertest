...
<div class="row">
  ...
  <tr>
        ...
        <form class="trip-form" action="{{route('cost.update', $data->ambulance->id)}}" method="post">
          @csrf
      <td  class="table-warning">@if($data->status=='pending')<input type="number" id="frame" name="cost" value="{{$data->ambulance->cost}}" style="width: 8rem; padding-right: 20px;">  @endif</td>
     <td  class="table-warning">@if($data->status=='pending') <button type="submit" class="btn btn-primary">Submit</button> @endif</td>
</form>
